 ...
 */
export function mapToSVG({ statMap, viewPortWidth, viewPortHeight }: SVGData): SVGSVGElement {
    ...
    statMap.getLayers().forEach((layer: BaseLayer) => {
        if (layer instanceof VectorLayer) {
            ...
            features.forEach(feature => {
                ...
                let pathString: string = __createPathString(geometry, statMap);
 ...
 * It is used internally in the mapToSVG function.
 * 
 * @param geometry 
 * @param statMap 
 * @returns 
 */
function __createPathString(geometry: Geometry, statMap: Map): string {
 ...
 * @returns 
 */
export async function mapToVideo(svgdata: SVGData, tsdata: TimeSeriesData, resolution: Resolution) {
    try {
        const svg: SVGSVGElement = mapToSVG(svgdata);
